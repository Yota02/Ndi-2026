...
// --- CONFIG BOTS ---
const BOT_SPAWN_RATE = 5000
const BOT_SPEED = 2.5
const BOT_SIZE = 40
const BOT_FIRE_RATE = 2500
const BULLET_SPEED = 7
const BULLET_SIZE = 12
const BOT_MAX_SPEED = 3.5
const BOT_WANDER_SPEED = 1.5
const BOT_PANIC_DIST = 350
const BOT_FRICTION = 0.95

// --- CONFIG SCORE ---
const SCORE_POLLUTION = 100
const SCORE_BOT = 300
const SCORE_NATURE_PENALTY = 50
...
const GUN_OFFSETS = {
  right: { x: 25, y: 23 }, left: { x: -25, y: -5 }, down: { x: -10, y: 15 }, up: { x: 45, y: -8 }
}
...
const score = ref(0) // <--- NOUVEAU : Score
...
// --- JOUEUR ---
const player = ref({ x: window.innerWidth / 2, y: window.innerHeight / 2, angle: 0, radius: 25 })
...
const beam = ref({ active: false, head: 0, tail: 0, state: 'idle' as 'idle'|'shooting'|'finishing', angle: 0, startX: 0, startY: 0 })

// --- ENTITÉS & BOTS ---
interface GameEntity {
  id: number; x: number; y: number; vx: number; vy: number;
  type: 'tree' | 'pollution'; emoji: string; hit: boolean;
}
const entities = ref<GameEntity[]>([])

interface BotEntity {
  id: number; x: number; y: number; vx: number; vy: number;
  lastShot: number; changeDirTimer: number;
}
...
const spriteStyle = computed(() => {
  const sheetW = 714; const sheetH = 349; const cols = 5; const rows = 4
  const frameW = sheetW / cols; const frameH = sheetH / rows
  const xPos = -(animState.value.frameIndex * frameW)
  const yPos = -(animState.value.directionRow * frameH)
  const scale = 0.9; const scaleX = animState.value.flipX ? -scale : scale
  return {
    backgroundImage: `url(${currentSprite.value})`, width: `${frameW}px`, height: `${frameH}px`,
    backgroundSize: `${sheetW}px ${sheetH}px`, backgroundPosition: `${xPos}px ${yPos}px`,
    position: 'absolute' as const, top: '50%', left: '50%',
    transformOrigin: 'center center', transform: `translate(-50%, -50%) scale(${scaleX}, ${scale})`,
    imageRendering: 'pixelated' as const, zIndex: 0
  }
})
...
const spawnEntity = () => {
  ...
  entities.value.push({
    ...
    vx: Math.cos(angle) * ENTITY_SPEED, vy: Math.sin(angle) * ENTITY_SPEED,
    type: isPollution ? 'pollution' : 'tree', emoji, hit: false
  })
}

const spawnBot = () => {
  if (isGameOver.value || !isGameStarted.value) return
  const x = Math.random() > 0.5 ? Math.random() * window.innerWidth : (Math.random() > 0.5 ? 0 : window.innerWidth);
  const y = Math.random() * window.innerHeight;
  bots.value.push({
    id: performance.now() + Math.random(),
    x: Math.random() * (window.innerWidth - BOT_SIZE), y: Math.random() * (window.innerHeight - BOT_SIZE),
    vx: 0, vy: 0, lastShot: performance.now(), changeDirTimer: 0
  })
}

// --- INPUTS & TIR ---
const handleKeyDown = (e: KeyboardEvent) => {
  const k = e.key.toLowerCase(); if (k in keys) keys[k as keyof typeof keys] = true; if (e.key === 'Escape') emit('close')
}
const handleKeyUp = (e: KeyboardEvent) => { const k = e.key.toLowerCase(); if (k in keys) keys[k as keyof typeof keys] = false }
const handleMouseMove = (e: MouseEvent) => { mousePos.value.x = e.clientX; mousePos.value.y = e.clientY }
...
const shoot = () => {
  ...
  let offset = GUN_OFFSETS.right
  if (deg > 45 && deg <= 135) offset = GUN_OFFSETS.down
  else if (deg > -135 && deg <= -45) offset = GUN_OFFSETS.up
  else if (deg > 135 || deg <= -135) offset = GUN_OFFSETS.left

  beam.value.active = true; beam.value.state = 'shooting'
  beam.value.head = 0; beam.value.tail = 0; beam.value.angle = aimAngle
  beam.value.startX = offset.x; beam.value.startY = offset.y
}
...
const checkLaserCollision = (targetX: number, targetY: number, radius: number) => {
  ...
  const nearestX = startX + t * dx; const nearestY = startY + t * dy
...
const update = (timestamp: number) => {
  ...
  // 2. Joueur
  let pdx = 0, pdy = 0
  if (keys.z) pdy -= 1; if (keys.s) pdy += 1; if (keys.q) pdx -= 1; if (keys.d) pdx += 1
  const isMoving = pdx !== 0 || pdy !== 0
  if (!beam.value.active && isMoving) player.value.angle = Math.atan2(pdy, pdx)
  if (isMoving) {
    const speed = 5; const moveSpeed = (pdx !== 0 && pdy !== 0) ? speed / 1.414 : speed
    const nx = player.value.x + pdx * moveSpeed; const ny = player.value.y + pdy * moveSpeed
    if (nx > 0 && nx < window.innerWidth) player.value.x = nx
    if (ny > 0 && ny < window.innerHeight) player.value.y = ny
  ...
  } else animState.value.frameIndex = 0
  updateSpriteDirection()
  ...
  if (beam.value.active) {
    if (beam.value.state === 'shooting') {
      ...
      if (beam.value.head >= BEAM_MAX_LENGTH) { beam.value.head = BEAM_MAX_LENGTH; beam.value.state = 'finishing' }
    } else if (beam.value.state === 'finishing') {
      beam.value.tail += BEAM_SPEED
      if (beam.value.tail >= beam.value.head) { beam.value.active = false; beam.value.state = 'idle' }
    }
  }
  ...
  for (let i = entities.value.length - 1; i >= 0; i--) {
    ...
    if (ent.x < 0 || ent.x > window.innerWidth - ENTITY_SIZE) ent.vx *= -1
    if (ent.y < 0 || ent.y > window.innerHeight - ENTITY_SIZE) ent.vy *= -1

    if (beam.value.active && !ent.hit && checkLaserCollision(ent.x + ENTITY_SIZE/2, ent.y + ENTITY_SIZE/2, ENTITY_SIZE/2)) {
      ent.hit = true
      if (ent.type === 'pollution') {
        temperature.value -= 0.1
        score.value += SCORE_POLLUTION // SCORE +
      } else {
        temperature.value += 0.1
        score.value = Math.max(0, score.value - SCORE_NATURE_PENALTY) // SCORE -
      }
  ...
  for (let i = bots.value.length - 1; i >= 0; i--) {
    ...
    const dx = bot.x - player.value.x; const dy = bot.y - player.value.y; const dist = Math.hypot(dx, dy)

    if (dist < BOT_PANIC_DIST) {
      const force = 0.2; bot.vx += (dx / dist) * force; bot.vy += (dy / dist) * force
    } else {
      if (timestamp - bot.changeDirTimer > 1000) {
        const randAngle = Math.random() * Math.PI * 2
        bot.vx += Math.cos(randAngle) * 0.5; bot.vy += Math.sin(randAngle) * 0.5
        bot.changeDirTimer = timestamp
      }
    }

    const margin = 100; const wallForce = 0.8
    if (bot.x < margin) bot.vx += wallForce
    if (bot.x > window.innerWidth - margin) bot.vx -= wallForce
    if (bot.y < margin) bot.vy += wallForce
    if (bot.y > window.innerHeight - margin) bot.vy -= wallForce

    bot.vx *= BOT_FRICTION; bot.vy *= BOT_FRICTION
    const speed = Math.hypot(bot.vx, bot.vy)
    const maxSpeed = (dist < BOT_PANIC_DIST) ? BOT_MAX_SPEED : BOT_WANDER_SPEED
    if (speed > maxSpeed) { bot.vx = (bot.vx / speed) * maxSpeed; bot.vy = (bot.vy / speed) * maxSpeed }
    bot.x += bot.vx; bot.y += bot.vy

    if (timestamp - bot.lastShot > BOT_FIRE_RATE) {
      const aimX = (player.value.x - bot.x); const aimY = (player.value.y - bot.y); const aimDist = Math.hypot(aimX, aimY)
      enemyProjectiles.value.push({
        id: performance.now() + Math.random(),
        x: bot.x + BOT_SIZE/2, y: bot.y + BOT_SIZE/2,
        vx: (aimX / aimDist) * BULLET_SPEED, vy: (aimY / aimDist) * BULLET_SPEED
      })
      bot.lastShot = timestamp
    }

    if (beam.value.active && checkLaserCollision(bot.x + BOT_SIZE/2, bot.y + BOT_SIZE/2, BOT_SIZE/2)) {
      bots.value.splice(i, 1)
      score.value += SCORE_BOT // SCORE +++
    }
  }
  ...
  for (let i = enemyProjectiles.value.length - 1; i >= 0; i--) {
    ...
    if (proj.x < -50 || proj.x > window.innerWidth + 50 || proj.y < -50 || proj.y > window.innerHeight + 50) {
      enemyProjectiles.value.splice(i, 1); continue
    }
    if (checkCircleCollision({x: proj.x, y: proj.y, r: BULLET_SIZE/2}, {x: player.value.x, y: player.value.y, r: player.value.radius})) {
      temperature.value += 0.1
  ...
  if (temperature.value < MIN_TEMP) temperature.value = MIN_TEMP
...
onMounted(() => {
  ...
  const img1 = new Image(); img1.src = sadTuxSprite
  const img2 = new Image(); img2.src = cookedChickenImg
  ...
  const interval = setInterval(() => {
    ...
    if (countdown.value <= 0) {
      ...
      botSpawnerId.value = window.setInterval(spawnBot, BOT_SPAWN_RATE)
    }
  }, 1000)
})
...
const screenBackground = computed(() => {
  const opacity = 0.85
  let r = 0, b = 0
  if (temperature.value > START_TEMP) r = Math.floor((temperature.value - START_TEMP) / (MAX_TEMP - START_TEMP) * 150)
  else b = Math.floor((START_TEMP - temperature.value) / START_TEMP * 150)
  return `rgba(${r}, 0, ${b}, ${opacity})`
})
const tempColor = computed(() => temperature.value >= 1.8 ? '#ff4444' : temperature.value <= 0.5 ? '#4444ff' : '#ffffff')
</script>

<template>
  <div class="laser-overlay" :style="{ background: screenBackground }" @mousedown="shoot">

    <div class="hud-score">SCORE: {{ score }}</div>

    <div class="hud-temp" :style="{ color: tempColor }">
      Temp: +{{ temperature.toFixed(1) }}°C
      <div class="temp-bar"><div class="temp-fill" :style="{ width: (temperature/MAX_TEMP*100)+'%', background: tempColor }"></div></div>
    </div>
    ...
    <div class="player-wrapper" :style="{ transform: `translate(${player.x}px, ${player.y}px)` }">
      <div class="laser-pivot" :style="{ transform: `translate(${beam.startX}px, ${beam.startY}px) rotate(${beam.angle}rad)`, zIndex: laserZIndex }">
        <div v-if="beam.active" class="laser-beam" :style="{ left: beam.tail+'px', width: (beam.head-beam.tail)+'px' }">
          <div class="laser-core"></div><div class="laser-glow"></div>
        </div>
      </div>
      <div :style="spriteStyle"></div>
    </div>

    <div v-for="ent in entities" :key="ent.id" class="entity" :class="ent.type"
         :style="{ transform: `translate(${ent.x}px, ${ent.y}px)`, width: ENTITY_SIZE+'px', height: ENTITY_SIZE+'px' }">
      {{ ent.emoji }}
    </div>
...
<style scoped>
.laser-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 9999; cursor: crosshair; user-select: none; }

/* HUD */
.hud-temp { position: absolute; top: 20px; right: 20px; font-family: monospace; font-size: 2rem; font-weight: bold; background: rgba(0,0,0,0.5); padding: 15px; border-radius: 8px; z-index: 200; }
.hud-score {
  position: absolute; top: 20px; left: 20px;
  font-family: monospace; font-size: 2rem; font-weight: bold; color: #ffd700; text-shadow: 2px 2px 0 #000;
  background: rgba(0,0,0,0.5); padding: 15px; border-radius: 8px; z-index: 200;
}
...
/* GAME OVER */
.game-over { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0,0,0,0.95); padding: 40px; text-align: center; border: 4px solid red; border-radius: 20px; color: white; z-index: 300; min-width: 400px; }
...
/* COUNTDOWN */
.countdown-overlay { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center; z-index: 400; pointer-events: none; }
